...
export async function POST(req: NextRequest) {
  try {
    ...
    const prompt = `Extraia todas as informações desta carta de condução portuguesa e retorne em formato JSON com estes campos:
{
...
Retorne APENAS JSON válido, sem texto adicional.`
    ...
    return NextResponse.json(
      { error: 'Erro ao extrair dados da carta de condução' },
      { status: 500 }
    )
  }
}
